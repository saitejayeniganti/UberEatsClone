...
exports.getOrdersForRestaurant = `select Orders.id,Orders.customer_id,Orders.restaurent_id,
Orders.price,Dishes.price as dish_price,Dishes.name as dish_name, 
Orders.order_date,Orders.delivery_type,Orders.order_status,Customer.name,
Customer.email_id,Customer.address,Order_items.dish_id,Order_items.quantity from Orders 
inner join Customer on Customer.id=Orders.Customer_id 
inner join Order_items on Orders.id=Order_items.order_id i
nner join Dishes on Order_items.dish_id=Dishes.id where Orders.restaurent_id=? 
order by FIELD(Orders.order_status,'Placed','Confirmed','ReadyToBePicked','Delivered' ),Orders.order_date desc`;
